...
from psycopg2 import \
    OperationalError as \
    Psycopg2Error  # Importing PostgreSQL OperationalError for simulation


@patch(
    "core.management.commands.wait_for_db.Command.check"
)  # this command will use the patch decorator to mock database check method
class CommandTests(SimpleTestCase):
    ...
    def test_wait_for_db_delay(self, patched_sleep, patched_check):
        ...
        )  # Simulate delays in database readiness

        call_command("wait_for_db")  # Call the custom command

        self.assertEqual(
            patched_check.call_count, 6
        )  # Assert the check method was called six times
        patched_check.assert_called_with(
            databases=["default"]
        )  # Assert the check method was called with the correct database
